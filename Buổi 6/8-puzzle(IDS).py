...
# ═══════════════════════════════════════════════════════════════
#  LOGIC THUẬT TOÁN IDS CHO 8-PUZZLE
# ═══════════════════════════════════════════════════════════════
def ACTIONS(state):
    ...
    random.shuffle(moves)                   # Xáo trộn để IDS có tính ngẫu nhiên
    return moves
...
# ─── Depth-Limited Search (DLS) – nền tảng của IDS ───────────
def DEPTH_LIMITED_SEARCH(initial_state, goal_state, limit, explore_log):
    """DFS có giới hạn độ sâu. Trả về (solution | 'cutoff' | None)."""
    node = {'STATE': initial_state, 'parent': None, 'action': None, 'depth': 0}
    return _RECURSIVE_DLS(node, goal_state, limit, explore_log, set())

def _RECURSIVE_DLS(node, goal_state, limit, explore_log, visited):
    """Đệ quy DLS – trả về solution, 'cutoff', hoặc None."""
    # Ghi log khám phá
    explore_log.append({
        'node':           node,
        'frontier_size':  0,
    ...
    for action in ACTIONS(node['STATE']):
        ...
        if child['STATE'] in visited:
            continue
        result = _RECURSIVE_DLS(child, goal_state, limit, explore_log, visited)
        if result == 'cutoff':
...
def ITERATIVE_DEEPENING_SEARCH(initial_state, goal_state, max_depth=50):
    """Thuật toán IDS – kết hợp ưu điểm của BFS (tối ưu) và DFS (tiết kiệm bộ nhớ).
    Lặp DLS với giới hạn độ sâu tăng dần từ 0 đến max_depth."""
    explore_log   = []
    depth_history = []          # Ghi lại số node khám phá tại mỗi depth limit
    for depth in range(max_depth + 1):
        nodes_before = len(explore_log)
        result = DEPTH_LIMITED_SEARCH(initial_state, goal_state, depth, explore_log)
        nodes_at_depth = len(explore_log) - nodes_before
        depth_history.append((depth, nodes_at_depth))
        if result != 'cutoff' and result is not None:
            return result, explore_log, depth_history
    return None, explore_log, depth_history
...
class PuzzleApp:
    def __init__(self, root):
        ...
        self.depth_history  = []     # Lịch sử (depth_limit, nodes_explored)
    ...
    def _build_ui(self):
        ...
        tk.Label(header, text="8-PUZZLE SOLVER (IDS)",
        ...
        tk.Label(left, text="INITIAL STATE",
                font=('Consolas', 9, 'bold'), bg='#f0f0f0').pack(pady=(10, 0))
        self.init_cells = self._create_grid(left, [1, 2, 3, 4, 0, 6, 7, 5, 8])
        tk.Label(left, text="GOAL STATE",
                font=('Consolas', 9, 'bold'), bg='#f0f0f0').pack(pady=(15, 0))
        self.goal_cells = self._create_grid(left, [1, 2, 3, 4, 5, 6, 7, 8, 0])
        tk.Label(left, text="ALGORITHM",
                font=('Consolas', 9, 'bold'), bg='#f0f0f0').pack(pady=(15, 0))
        self.algo_var = tk.StringVar(value='IDS')
        tk.Radiobutton(left, text="IDS (Iterative Deepening)",
                    variable=self.algo_var, value='IDS',
                    bg='#f0f0f0', font=('Consolas', 9)).pack(anchor='w', padx=20)
        # Max depth cho IDS
        tk.Label(left, text="MAX DEPTH",
                font=('Consolas', 9, 'bold'), bg='#f0f0f0').pack(pady=(10, 0))
        self.maxdepth_var = tk.IntVar(value=50)
        tk.Scale(left, from_=10, to=150, orient='horizontal',
                variable=self.maxdepth_var,
                bg='#f0f0f0', highlightthickness=0).pack(fill='x', padx=20)
        # Thanh trượt tốc độ
        tk.Label(left, text="SPEED (ms)",
                font=('Consolas', 9, 'bold'), bg='#f0f0f0').pack(pady=(10, 0))
        ...
        self.stats_lbl = tk.Label(left, text="Ready",
                                bg='#f0f0f0', font=('Consolas', 8), justify='left')
        ...
        m_frame.pack(fill='x')
        tk.Radiobutton(m_frame, text="Solution Path",
                    variable=self.mode_var, value='solution',
                    command=self._refresh_display, bg='#ffffff').pack(side='left')
        tk.Radiobutton(m_frame, text="Exploration Log",
                    variable=self.mode_var, value='explore',
                    command=self._refresh_display, bg='#ffffff').pack(side='left')
        # Bảng 3×3 hiển thị
    ...
    def _read_grid(self, cells):
        ...
        try:
            ...
            messagebox.showerror("Lỗi",
                                "Vui lòng nhập đúng các số từ 0 đến 8 (không trùng lặp).")
            return None

    # ─── Giải ────────────────────────────────────────────────
    def _solve(self):
        """Thực hiện giải bài toán bằng IDS."""
        self._stop_play()
        ...
        max_d = self.maxdepth_var.get()
        self.log_box.delete('1.0', 'end')
        self.log_box.insert('end',
            f"Solving IDS (max_depth={max_d})...\n"
            f"From: {init}\nTo  : {goal}\n")
        self.root.update()
        sol, log, depth_hist = ITERATIVE_DEEPENING_SEARCH(init, goal, max_d)
        self.solution_path = sol or []
        self.explore_log   = log
        self.depth_history = depth_hist
        self.current_step  = 0
        if sol:
            # Tìm depth_limit cuối cùng (lúc thành công)
            final_depth = depth_hist[-1][0] if depth_hist else '?'
            ...
            # Ghi log theo từng depth iteration
            self.log_box.insert('end', "\n── Depth iterations ──\n")
            for d, n in depth_hist:
                self.log_box.insert('end', f"  depth={d:3d}  nodes={n}\n")
        else:
            self.stats_lbl.config(
                text=(f"✕ No solution found\n"
                    f"Nodes: {len(log)}\n"
                    f"MaxDepth: {max_d}"))
            messagebox.showwarning("Kết quả",
                                "Không tìm thấy đường đi giữa hai trạng thái này.")
        self._refresh_display()

    # ─── Hiển thị ────────────────────────────────────────────
    def _refresh_display(self):
        """Cập nhật giao diện bảng và thông tin theo bước hiện tại."""
        lst = (self.solution_path
            if self.mode_var.get() == 'solution'
            else self.explore_log)
        if not lst:
            return
        ...
        for i in range(3):
            for j in range(3):
                ...
                self.board_labels[i][j].config(
                    ...
                    bg=('#e0e0e0' if is_correct
                        else ('#999999' if val == 0 else '#ffffff'))
                )
        self.info_lbl.config(
            text=(f"Step {self.current_step} / {len(lst) - 1} | "
