...
class PuzzleApp:
    ...
    def _solve(self):
        ...
        if sol is None:
            ...
            if final_state != self.target_goal:
    ...
    def _refresh_display(self):
        ...
        if algo in ['Greedy_Search', 'Simple_Hill_Climbing', 'Steepest_Ascent_Hill_Climbing', 
                    'Stochastic_Hill_Climbing', 'Random_Restart_Hill_Climbing', 'Local_Beam_Search']:
        ...
        for idx, item in enumerate(step_data['frontier_list'], 1):
            if isinstance(item, tuple):
                if len(item) == 3:
                    ...
                    s, score = item
                    label_char = 'h' if algo == 'Greedy_Search' else ('f' if algo in ['A*', 'IDA*'] else 'Cost')
