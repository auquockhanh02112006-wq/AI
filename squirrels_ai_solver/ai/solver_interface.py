...
from ai.complex.no_observation_search import no_observation_solve

# Danh sách thuật toán theo bảng yêu cầu:
# 1. Tìm kiếm mù (Uninformed)  → BFS, IDS, DFS
# 2. Tìm kiếm có thông tin     → A*, Greedy, IDA*
# 3. Local Search              → Hill Climbing, Simulated Annealing, Local Beam
# 4. Adversarial               → Minimax, Alpha-Beta, Expectimax
# 5. CSP                       → Backtracking, Min-Conflicts, AC-3
# 6. Complex (Không quan sát)  → AND-OR Search, Belief-State, No Observation
ALGORITHMS = {
    "BFS":                 bfs_solve,
    "IDS":                 ids_solve,
    "DFS":                 dfs_solve,
    ...
    "No Observation":      no_observation_solve,
}
