...
class PuzzleApp:
    ...
    def _solve(self):
        ...
        if sol is None:
            ...
            if algo not in ['Sensorless_Conformant_A*', 'Completely_Blind_BFS', 'Partially_Observable_BFS', 'AND_OR_GRAPH_SEARCH'] and final_state != self.target_goal:
    ...
    def _refresh_display(self):
        ...
        self.info_depth_lbl.config(text=f"Depth: {current_node.get('depth', 0)}")
        ...
        for idx, item in enumerate(step_data['frontier_list'], 1):
            if isinstance(item, tuple):
                if len(item) == 3: s, score, note = item; self.list_frontier.insert(tk.END, f" {idx:02d}. {s} [h={score}] : {note}")
                elif len(item) == 2:
                    s, score = item
