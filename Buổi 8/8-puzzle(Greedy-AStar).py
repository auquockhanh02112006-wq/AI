...
def MANHATTAN_DISTANCE(state, goal):
    ...
    for i in range(9):
        val = state[i]
        if val != 0:  # Thông thường không tính khoảng cách cho ô trống (0)
...
def GREEDY_SEARCH_ALGO(initial_state, goal_state):
    """Thuật toán tìm kiếm tham lam (Greedy Best-First Search) theo slide"""
    explore_log = []
    
    # 1. Khởi tạo tập FRONTIER = {Start}. Tính hàm đánh giá Start: h(Start)
    start_node = {
        'STATE': initial_state, 
        'parent': None, 
        'action': None, 
    ...
    while FRONTIER:
        ...
        if n['STATE'] == goal_state:
            explore_log.append({
                'node': n,
                'frontier_list': [(item['STATE'], item['h']) for item in FRONTIER],
        ...
        for action in ACTIONS(n['STATE']):
            ...
            if not in_frontier and not in_reached:
                # Gán đỉnh cha của m là n (Đã xử lý trong CHILD_NODE)
...
def A_STAR_SEARCH_ALGO(initial_state, goal_state):
    """Mô tả thuật toán A* theo slide"""
    explore_log = []
    ...
    while FRONTIER:
        ...
        if n['STATE'] == goal_state:
            explore_log.append({
                'node': n,
                'frontier_list': [(item['STATE'], item['f']) for item in FRONTIER],
                'explored_list': list(REACHED.keys())
            })
            return SOLUTION(n), explore_log
        ...
        for action in ACTIONS(n['STATE']):
            ...
            g_new = n['g'] + 1  # Mỗi bước dịch chuyển ô số có cost = 1
...
class PuzzleApp:
    ...
    def _build_ui(self):
        ...
        # Đặt tên danh sách lựa chọn khớp với bài giảng của bạn
        self.algo_cb = ttk.Combobox(ctrl_grid, textvariable=self.algo_var, 
                                    values=["BFS", "DFS", "IDS", "UCS", "Greedy_Search", "A*"], 
                                    state="readonly", width=14, font=('Consolas', 11))
    ...
    def _solve(self):
        ...
        if algo == 'BFS':
            ...
            sol, log = GREEDY_SEARCH_ALGO(init, goal)
        elif algo == 'A*':
            sol, log = A_STAR_SEARCH_ALGO(init, goal)
    ...
    def _refresh_display(self):
        ...
        # Cập nhật nhãn chi phí tùy thuộc thuật toán được chọn
        algo = self.algo_var.get()
